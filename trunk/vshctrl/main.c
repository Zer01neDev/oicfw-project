...
SEConfig config;
SceModule2 *fmod;
char *umdfile; // 0x00004CA8
void *var9; // Unknown
int unk_1 = 0; //0x00004CA9
char *unk2;//0x00004C74
SceKernelLMOption Satelite_LMOptions; // 0x00004B28
...
int timelow1/*0x00004CA0*/, timelow2/*0x00004B40*/, timelow3, timelow4;
int set;
VirtualPbp vpbp;
...
#if _PSP_FW_VERSION == 500
	int (*loc_0000DC18)(void *arg1, void *arg2);
#endif
...
int LoadExecVSHCommonPatched(int apitype, char *file, struct SceKernelLoadExecVSHParam *param)
{
		...
		else
		{
			...
			else
			{
				if(strstr(file, "ms0:/PSP/GAME/UPDATE/") != 0)
					goto label26;
				if(config.umdactivatedplaincheck == 0)
					param->args = strlen(param->argp)+1;				
				goto label39;
			}
		}
	...
	else
	{
		...
		param->argp = file;
		sctrlSESetUmdFile(virtualpbp_getfilename(index));
...
int vshCtrlReadBufferPositivePatched(SceCtrlData *pad_data, int count)
{
	...
	{
		if(!sceCtrlReadBufferPositiveReal)
		{
			if(vshmenu_mod >= 0)
			{
			...
			if(!sceKernelFindModuleByName("sceVshOSK_Module"))
			{
				if(!sceKernelFindModuleByName("camera_plugin_module"))
				{
					if(pad_data->Buttons & PSP_CTRL_SELECT)
					{
						...
						Satelite_LMOptions.size = sizeof(Satelite_LMOptions);
						Satelite_LMOptions.mpiddata = 5;
						Satelite_LMOptions.mpidtext = 5;
						Satelite_LMOptions.position = 0;
						Satelite_LMOptions.access = 1;
						vshmenu_mod = sceKernelLoadModule("flash0:/vsh/module/satelite.prx", 0, &Satelite_LMOptions);
						if(vshmenu_mod >= 0)
						{
							char* umd_file = 0;
							int umd_file_len = 0;
							if(umdfile)
							{
								...
								umd_file_len = strlen(umd_file) + 1;
							}
							sceKernelStartModule(vshmenu_mod, umd_file_len, umd_file, NULL, NULL);
...
// OK
void sceResmgr_driver_9DC14891Patched(char *arg1, int arg2, int arg3)
{
	sceResmgr_driver_9DC14891Real(arg1, arg2, arg3);
	int k1 = pspSdkSetK1(0);
	...
	if (strstr(arg1, "release:"MAJOR_VERSION"."MINOR_VERSION) != NULL)
	{
		...
		if (versiontxt >= 0)
		{
			int bytes = sceIoRead(versiontxt, arg1, arg2);
			((int *) arg3)[0] = bytes;
