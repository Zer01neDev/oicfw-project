/*
	...
	sctrlIsofs_Driver.c: SystemControl IsoFS Driver Code
...
Iso9660DirectoryRecord main_record;//0x0000ABC4
FileHandle handlers[MAX_HANDLERS];//handlers = 0x0000A7A0 // handlers[MAX_HANDLERS] = 0x0000A7C0
u8 *sectors;//0x0000A794

SceUID isofs_sema;//0x0000A798
int g_lastLBA;//0x0000A79C
int g_lastReadSize;//0x0000ABE8

extern int Umd9660ReadSectors2(int lba, int nsectors, void *buf);
...
int IsofsReadSectors(int lba, int nsectors, void *buf)
{
	...
	if (buf != sectors)
	{
		ret = Umd9660ReadSectors2(lba, nsectors, buf);
	}
	else
	{
		if (((nsectors < 9)) == 0)
		{
			ret = SCE_ERROR_ERRNO_EFBIG;
		}
		else
		{
			memset((buf+(nsectors*SECTOR_SIZE), 0, 64);
			ret = Umd9660ReadSectors2(lba, nsectors, buf);
		}
	}
	return ret;
...
int isofs_init()
{
	...
	sectors = (u8 *)oe_malloc(SIZE_OF_SECTORS);
	if (!sectors)
	{
		return -1;
	}
...
typedef struct
{
	PspIoDrvFileArg *arg;//0x0000ABA0
	char *file;//0x0000ABA4
	int flags;//0x0000ABA8
	SceMode mode;//0x0000ABAC
} OpenParams;
...
// OK
void isofs_close(PspIoDrvFileArg *arg)
{
...
typedef struct
{
	PspIoDrvFileArg *arg;//0x0000ABEC
	char *data;//0x0000ABF0
	int len;//0x0000ABF4
} ReadParams;
	...
	else
	{
		...
		if (whence != PSP_SEEK_SET)
		{
			if (whence != PSP_SEEK_CUR)
			{
				...
				{
					var24 = handlers[fd].filesize - (int)ofs;
					handlers[fd].filepointer = handlers[fd].filesize - (int)ofs;
					sceKernelSignalSema(isofs_sema, 1);
					ret = handlers[fd].filepointer;
					var13 = handlers[fd].filepointer >> 0x0000001F;
				}
			}
			else
			{
				var24 = handlers[fd].filepointer + (int)ofs;
				handlers[fd].filepointer = handlers[fd].filesize - (int)ofs;
				sceKernelSignalSema(isofs_sema, 1);
				ret = handlers[fd].filepointer;
				var13 = handlers[fd].filepointer >> 0x0000001F;
...
SceOff isofs_lseek(PspIoDrvFileArg *arg, SceOff ofs, int whence)
{
	return isofs_lseek(arg, ofs, whence);
}

typedef struct
{
	PspIoDrvFileArg *arg;//0x0000ABB8
	const char *file;//0x0000ABBC
	SceIoStat *stat;//0x0000ABC0
} GetStatParams;
...
typedef struct
{
	PspIoDrvFileArg *arg;//0x0000ABB0
	SceIoDirent *dirent;//0x0000ABB4
} DreadParams;
...
int isofs_ioctl(PspIoDrvFileArg *arg, unsigned int cmd, void *indata, int inlen, void *outdata, int outlen)
{
	...
	switch (cmd)
	{
		case 0x01020004: // SCE_ISOFS_GET_FILEPOINTER
			...
			outdata[0] = handlers[fd].filepointer;
			sceKernelSignalSema(isofs_sema, 1);
			return 0;
		break;
		...
		case 0x01020007: // get file size (64 bits)
			...
			outdata[0] = handlers[fd].filesize;
			outdata[1] = 0;
			sceKernelSignalSema(isofs_sema, 1);
			return 0;
		break; 		
...
{
	return &isofs_driver;//0x0000A1F0
}
